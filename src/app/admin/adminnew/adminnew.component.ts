...
export class AdminnewComponent {
  displayedColumns = ['name','address','date/time','ordervalue','no','invoice'];
  date;
  selected = '11';
...
this.date = new FormControl(moment([f, e, d]));
  ...
  cusdetail(a){
    ...
    this.customer = docRef$.ref.get().then(function (doc) {
     if (doc.exists) {
...
//agent address
    ...
    doc.text(a,500,160);;
     doc.text("Scheduled Date :",440,170);
    doc.text(g,500,170);
     doc.text("Order Date :",440,180);
    doc.text(h,500,180);
...
doc.save('SonderWorks.pdf');
  ...
  update(a:any,b:any){
    console.log(a,b);
    this.afs.collection('orders').doc(b).update({
     status: 'pending'
   }).then(() => {
     
   })
   this.afs.collection(`users/${a}/orders`).doc(b).update({
    status: 'pending'
  }).then(() => {
    alert('updated');
  })
 ...
 delete(c:any,d:any) {
  this.afs.collection('orders').doc(d).delete().then(() => {
   
  })
  this.afs.collection(`users/${c}/orders`).doc(d).delete().then(() => {
    alert('deleted');
  })
}
download(a,b,c,d,e){
